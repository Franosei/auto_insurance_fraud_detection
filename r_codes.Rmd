...
We will first split our dataset into training and test data. We will use 75\% of our data for training and 25\% for testing our machine learning model
```{r}
set.seed(123)
...
sub_tag <- tag %>% group_by(fraud_reported) %>% 
  mutate(count = n()/1000*100) %>% 
  distinct() 
pie3D(sub_tag$count,radius=1.5,labels = c("24.7%","75.3%"),explode = 0.1, col=c("orange", "darkred"))
legendg(0.3,1.2,c("fraud","not fraud"),fill=c("orange", "darkred"))
```
